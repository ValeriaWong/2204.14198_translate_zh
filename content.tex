...
\noindent
\textbf{Limitations.}首先，我们的模型建立在预训练 LM 的基础上，因此直接继承了 LM 的弱点。 例如，LM 先验通常是有帮助的，但偶尔也会产生幻觉和无根据的猜测。 此外，LM 对于比训练序列更长的序列的泛化能力也很差。 在训练过程中，它们的样本效率也很低。 解决这些问题可以加快该领域的进展，并增强 Flamingo 等 VLM 的能力。
