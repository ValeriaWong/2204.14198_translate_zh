...
如（i）行所示，获取正确的训练数据至关重要。 事实上，移除交错的图像-文本数据集 \mmmw{} \emph{导致性能下降了超过$17\%$}
同时移除传统的配对图像-文本对也会导致性能下降(by $9.8\%$)，
这表明需要不同类型的数据集。 此外，移除我们的配对视频-文本数据集也会对所有视频任务的性能产生负面影响。
 我们在行（ii）中对比了使用轮换更新~\citep{cho2021unifying} 和我们的梯度累积策略的重要性，结果显示我们的梯度累积策略更为重要。同时，我们在（ii）行中显示了将我们的图像-文本对（ITP）替换为公开可用的 LAION-400M 数据集~\cite{schuhmann2021laion} 的结果，结果显示性能略有下降。
