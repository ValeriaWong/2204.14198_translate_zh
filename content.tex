...
\noindent
\textbf{Limitations.} 首先，我们的模型建立在预训练语言模型（LMs）之上，并因此直接继承了它们的弱点。
例如，语言模型的先验知识通常是有益的，但有时可能会导致幻觉（hallucinations）或无根据的猜测。
此外，语言模型在处理比训练序列更长的序列时泛化能力较差。
它们在训练过程中也存在样本效率低的问题。
解决这些问题能够加速该领域的进展，并提升类似 Flamingo 这样的视觉-语言模型（VLMs）的能力。
...
\noindent
\textbf{社会影响。}
在社会影响方面， \largem{} 带来诸多益处的同时也伴随着一些风险。
它能够快速适应广泛多样的任务，使非专业用户在数据稀缺的情况下也能获得良好的表现，从而降低了有益和恶意应用的门槛。
\largem{} 面临与大型语言模型相同的风险，例如输出冒犯性语言、传播社会偏见和刻板印象，以及泄露私人信息~\citep{weidinger2021harms,chinchilla}.
此外，它在处理视觉输入方面的能力也带来了特定的风险，例如与输入图像内容相关的性别和种族偏见，这与许多视觉识别系统类似~\citep{hendricks2018women,zhao2021understanding,buolamwini2018gender,de2019does,schwemmer2020diagnosing}.
我们建议读者参阅附录~\maintoappref{sec:broader_impact} ，以了解更多关于我们工作的社会影响（包括正面和负面）的广泛讨论，以及缓解策略和针对种族或性别偏见及有害输出的早期风险研究。
最后我们注意到，效仿先前专注于语言模型的研究工作~\citep{thoppilan2022lamda,perez2022red,menick2022teaching}的少样本能力可能有助于缓解此类风险。
